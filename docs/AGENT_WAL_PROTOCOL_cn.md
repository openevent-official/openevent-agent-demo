...
- 每一次新的 `infer.request` attempt 前都 MUST 写入一条新的 WAL 记录；超时重试也是新的 `infer.request`，因此也需要新的 WAL 记录。
...
- `agent.wal.v1` 只做向后兼容增强。
- 破坏性变更使用新的 channel protocol，如 `agent.wal.v2`。
- 首版仅定义 `llm.request.prepare`；未来可新增其他 `kind`，但不得改变已定义字段语义。
